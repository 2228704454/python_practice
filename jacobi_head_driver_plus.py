...
class Steering_gear:  # 定义类
    ...
    def moveServo(self, servoID, position1, time):  # 构造函数 并定义形参舵机ID，目标位置，及转动时间
        ...
        buf[8] = position1 & 0xFF
        buf[9] = (position1 >> 8) & 0xFF  # 将position拆分为低八位和高八位
        self.serial.write(buf) 
        # serialX.write(buf, 10)
        # 在这里添加相应的串口通讯代码

    def moveServos(self, servoID, position2, time):  # 构造函数 并定义形参舵机ID，目标位置，及转动时间
        if servoID > 31 or time <= 0:  # 判断如果时间舵机ID大于31或者时间小于等于0，则返回函数不执行下面代码
            return
        ...
        buf[6] = (time >> 8) & 0xFF  # 将转动时间拆分为低八位与高八位
        buf[7] = servoID  # 将第七个元素设置为舵机ID
        buf[8] = position2 & 0xFF
        buf[9] = (position2 >> 8) & 0xFF  # 将position拆分为低八位和高八位
        self.serial.write(buf) 
    ...
    def angle_conversion1(self, angle1):
        min_position = 2100   # 舵机的最小位置值
        max_position = 3100  # 舵机的最大位置
        min_angle1 = 0  # 舵机的最小角度
        max_angle1 = 100  # 舵机的最大角度

        # 将角度映射到位置范围
        position = int((angle1 - min_angle1) / (max_angle1 - min_angle1) * (max_position - min_position) + min_position)

        return position

    def angle_conversion2(self, angle2):
        min_position = 1860  # 舵机的最小位置值
        max_position = 2860  # 舵机的最大位置
        min_angle2 = 0  # 舵机的最小角度
        max_angle2= 135  # 舵机的最大角度

        # 将角度映射到位置范围
        position = int((angle2 - min_angle2) / (max_angle2 - min_angle2) * (max_position - min_position) + min_position)
